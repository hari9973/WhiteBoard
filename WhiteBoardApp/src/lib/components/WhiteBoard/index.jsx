...
let origY;
let mouseDown = false;
let currentDrawingInstance = null;
...
// sockets way
const socket = io('/whiteboard', { transports: ['websocket'] });

socket.on('receiveData', (message) => {
  const figure = message;
  console.log('recieved message ', message);
  if (figure.type === 'line') {
    const drawinstance = new fabric.Line([figure.x1, figure.y1, figure.x2, figure.y2], {
    ...
    gCanvas.add(drawinstance);
  } else if (figure.type === 'rect') {
    // rect
    const drawinstance = new fabric.Rect({
      stroke: figure.stroke,
      strokeWidth: figure.strokeWidth,
      fill: figure.fill,
      left: figure.left,
    ...
    gCanvas.add(drawinstance);
  } else if (figure.type === 'ellipse') {
    const drawinstance = new fabric.Ellipse({
      stroke: figure.stroke,
      strokeWidth: figure.strokeWidth,
      fill: figure.fill,
      left: figure.left,
...
const initCanvas = (width, height) => {
  ...
  socket.on('receiveAllData', (message) => {
    const figures = message;
    console.log(figures);
...
function stopDrawing() {
  const json = JSON.stringify(currentDrawingInstance);
  console.log(json);

  // Sockets way
  socket.emit('sendData', JSON.parse(json));
  mouseDown = false;
}
...
function startAddLine(canvas) {
  return ({ e }) => {
    ...
    canvas.add(drawInstance);
    currentDrawingInstance = drawInstance;
